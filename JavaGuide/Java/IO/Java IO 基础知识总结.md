...
```
`input.txt` 文件内容：
![[Pasted image 20240723230457.png]]
输出：
```plain
...
运行结果：
![[Pasted image 20240723230718.png]]
...
![[Pasted image 20240723230826.png]]
...
`input.txt` 文件内容：
![[Pasted image 20240724000217.png]]
...
输出结果：
![[Pasted image 20240724000515.png]]
...
举个例子，我们可以 **通过 `BufferedInputStream`（字节缓冲输入流）来增强 `FileInputStream` 的功能。**
```java
// 新建一个 BufferedInputStream 对象
BufferedInputStream bufferedInputStream = new BufferedInputStream(new FileInputStream("input.txt"));
```
...
`BufferedOutputStream` 将数据（字节信息）写入到目的地（通常是文件）的过程中不会一个字节一个字节的写入，而是会先将要写入的字节存放在缓存区，并从内部缓冲区中单独写入字节。这样大幅减少了 IO 次数，提高了写入效率。
...
`input.txt` 文件内容：
![[Pasted image 20240724002040.png]]
...
![[Pasted image 20240724002112.png]]
