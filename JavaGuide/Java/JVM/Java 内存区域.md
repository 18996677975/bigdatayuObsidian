...
**JDK 1.7**：
![[Pasted image 20240701230308.png]]

**JDK 1.8**：
![[Pasted image 20240701230217.png]]
...
![[Pasted image 20240701230540.png]]
...
![[Pasted image 20240701230745.png]]
...
![[Pasted image 20240701230928.png]]
...
![[Pasted image 20240701231147.png]]
...
![[Pasted image 20240701231409.png]]
...
![[Pasted image 20240702002040.png]]

**为什么要将永久代 (PermGen) 替换为元空间 (MetaSpace) 呢?**
![[Pasted image 20240702002132.png]]
...
![[Pasted image 20240702002742.png]]
...
![[Pasted image 20240702003058.png]]
![[Pasted image 20240702003106.png]]
...
# 堆内内存、堆外内存、直接内存含义与区别
**堆内内存**： 
这是 JVM 管理的内存的主要部分，用于存储 Java 对象实例和数组。
当我们使用 `new` 关键字创建对象时，这些对象通常就分配在堆内内存中。例如创建一个 `User` 对象： 
```java
class User { 
  String name; 
  int age; 
...
User user = new User();
```
堆内内存的管理由 JVM 的垃圾回收机制负责，当对象不再被引用时，会被垃圾回收器回收以释放内存。

**堆外内存**： 堆外内存是指不在 JVM 堆中分配的内存。它是通过 Java 的 `ByteBuffer` 类或者使用一些第三方库（如 `Netty`）来分配和管理的。
堆外内存的使用可以减少内存复制，提高性能。比如在处理大量网络数据传输时：
```java 
ByteBuffer byteBuffer = ByteBuffer.allocateDirect(1024); 
``` 

**直接内存**： 直接内存也属于堆外内存的一种，但它在概念上更侧重于强调与 I/O 操作的直接交互。
使用直接内存可以避免在 Java 堆和 Native 堆之间来回复制数据，从而提高性能。例如在进行文件读取时，使用 `FileChannel` 的 `transferTo` 方法可以利用直接内存：
```java 
FileChannel fileChannel = new RandomAccessFile("file.txt", "rw").getChannel(); 
ByteBuffer directBuffer = ByteBuffer.allocateDirect(1024); 
fileChannel.transferTo(0, fileChannel.size(), directBuffer); 
```

**区别主要体现在以下几个方面：** 
1. 管理方式：堆内内存由 JVM 自动管理，包括内存的分配和回收；堆外内存和直接内存需要开发者手动管理内存的分配和释放。 
2. 垃圾回收：堆内内存受 JVM 垃圾回收机制影响；堆外内存和直接内存不受垃圾回收直接管理。 
3. 性能影响：堆外内存和直接内存可以减少内存复制，在一些特定场景（如大文件处理、网络通信等）下能提高性能。 
4. 内存溢出风险：堆内内存的溢出会被 JVM 捕获并抛出异常；而堆外内存和直接内存的溢出可能导致进程崩溃。
...
![[Pasted image 20240702005313.png]]

### 直接指针
如果使用直接指针访问，reference 中存储的直接就是对象的地址。
![[Pasted image 20240702005411.png]]
