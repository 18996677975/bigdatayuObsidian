...
![[Pasted image 20240701164018.png]]
对于存在前后顺序关系的接口调用，可以进行编排，如下图所示。
![[Pasted image 20240701164026.png]]
...
![[Pasted image 20240701164514.png]]
`CompletionStage` 接口描述了一个异步计算的阶段。很多计算可以分成多个阶段或步骤，此时可以通过它将所有步骤组合起来，形成异步计算的流水线。

`CompletableFuture` 除了提供了更为好用和强大的 `Future` 特性之外，还提供了函数式编程的能力。
![[Pasted image 20240701164553.png]]
...
![[Pasted image 20240701164637.png]]
...
通过创建一个结果值类型为 `RpcResponse<Object>` 的 `CompletableFuture`，可以把 `resultFuture` 看作是异步运算结果的载体。
```java
CompletableFuture<RpcResponse<Object>> resultFuture = new CompletableFuture<>();
```
假设在未来的某个时刻，得到了最终的结果，这时，调用 `complete()` 方法为其传入结果，这表示 `resultFuture` 已经被完成了。
```java
...
![[Pasted image 20240701172218.png]]
